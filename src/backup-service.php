<?PHP
namespace nadejdin\backupservice;
 
// SETTINGS START

define(BACKUPSERVICE_DATETIMEFORMAT, 'd/m/Y H:i:s');
define(BACKUPSERVICE_DATETIMEFORMATFORFILE, 'd-m-y-h-i-s');

class make{

    private $zip_file = '';
    private $db_file = '';
    private $filename = "ZipBackUp";
    private $in_dir = "";
    private $addTime = false;
    private $start_dir = "/";
    private $emails = [];
    private $full_dir = '';
	private $use_yii_mailer = false;
	private $smtpSetting = [];

	public function smtp($params){
		$this->smtpSetting = $params;
		return $this;
	}
	
    public function deleteBackup(){
        if( is_file( $this->full_dir ) == true )
            unlink( $this->full_dir );
    }

    public function sendTo( $email = false ){
        if( is_array($email) == false ){
            $this->emails[]=$email;
        } else if( is_array( $email ) == true ){
            foreach($email as $e_){
                $this->emails[]=$e_;
            }
        } else if($email == false){
    ...
    private function send_( $files = false ){
		if( $this->use_yii_mailer == true ){
			$message = \Yii::$app->mailer->compose()
			->setFrom([ \Yii::$app->params['SMTPEmail'] => 'Backup Service'])
			->setTo( $this->emails )
			->setSubject( 'Backup - ' . date(BACKUPSERVICE_DATETIMEFORMAT) )
			->setHtmlBody( '<h1>Backup - ' . date(BACKUPSERVICE_DATETIMEFORMAT)."</h1>" );
			if( is_array( $files ) == true ){
				foreach(  $files as $el_ )
					$message->attach( $el_ );   
			}else if( trim( $files ) != '' ){
			...
			$mail->SMTPAuth = true;  
			if(@$this->smtpSetting['debug']) $mail->SMTPDebug = 2;			
			$mail->Username = $this->smtpSetting['user'];                 
			$mail->Password = $this->smtpSetting['password'];                          
			$mail->SMTPSecure = $this->smtpSetting['secure'];                            
			$mail->Port = $this->smtpSetting['port'];                                    
			$mail->setFrom($this->smtpSetting['email_from'], 'Backup Service');
			foreach( $this->emails as $index=>$el )
				$mail->addAddress( trim($el), 'Receiver of backup');
			if( is_array( $files ) == true ){
				foreach(  $files as $el_ )
					$mail->addAttachment( $el_ );   
			}else if( trim( $files ) != '' ){
				$mail->addAttachment( $files );  
			}
			$mail->isHTML(true);     
			$mail->Subject = 'Backup - ' . date(BACKUPSERVICE_DATETIMEFORMAT);
			$mail->Body    = 'Backup - ' . date(BACKUPSERVICE_DATETIMEFORMAT);
			$mail->AltBody = 'Backup - ' . date(BACKUPSERVICE_DATETIMEFORMAT);
			if(!$mail->send()) {
				return false;
			} else {
				return true;
	...
	public function yiiMailer ($use = false){
		$this->use_yii_mailer = $use;
		return $this;
	}
	
    public function findTo($start_dir_ = false){
        $last = substr($start_dir_,(strlen($start_dir_)-1),1);
        $first = substr($start_dir_,0,1);
        $this->start_dir = (($first=="/")?$start_dir_:"/".$start_dir_);
        if( $last == "/" ) $this->start_dir = substr($this->start_dir,0,(strlen($this->start_dir)-1));
        if( is_dir( dirname(__FILE__)."/".$this->start_dir ) == false ) $this->start_dir = "";
        return $this;
    }
    ...
    public function db(){
		if( !@preg_match("/mysql:/",\Yii::$app->db->dsn) ) return $this;
		$params = explode(";",\Yii::$app->db->dsn);
		@list($trash,$dbserver) = spliti("=",$params[0],2);
		@list($trash,$dbname) = spliti("=",$params[1],2);
        $this->db_file = $this->backup_tables($dbserver,\Yii::$app->db->username,\Yii::$app->db->password,$dbname);
        return $this;
    }
    ...
    public function saveTo($dir = ''){
        ...
        $last = substr($dir,(strlen($dir)-1),1);
        $first = substr($dir,0,1);
        $this->in_dir = dirname(__FILE__) . (($first!="/")?"/":"") . $dir . (($last!="/")?"/":"");
        return $this;
    }

    public function filename($filename_="d-m-Y-H-i-s"){
    ...
    public function zip(){
        $zip = new Zip();
        $zip->setComment("Zip backup file. \nCreated on " . date(BACKUPSERVICE_DATETIMEFORMAT));
        $tree = $this->getDirs( $this->start_dir );
        if( count( $tree ) <= 0 ) return false;
		if( @$this->db_file ){
			$zip->addFile(file_get_contents($this->db_file), 
                    "/DB_BACKUP/".basename($this->db_file), 
                    filectime($this->db_file), 
                    NULL, 
                    TRUE, 
                    Zip::getFileExtAttr($tree[$i]));
		}
        for( $i=0; $i<count($tree); $i++ ){
            $tree[$i] = substr($tree[$i],1);
            $pathData = pathinfo($tree[$i]);
            $fileName = $pathData['filename'];
            if( is_file( dirname(__FILE__)."/".$tree[$i] ) == true )
                $zip->addFile(file_get_contents(dirname(__FILE__)."/".$tree[$i]), 
                    $tree[$i], 
                    filectime(dirname(__FILE__)."/".$tree[$i]), 
                    NULL, 
                    TRUE, 
                    Zip::getFileExtAttr($tree[$i]));
        }
        $this->filename = (($this->addTime==true)?date(BACKUPSERVICE_DATETIMEFORMATFORFILE)."-":"").$this->filename;
        $file_ = (($this->in_dir=='')?dirname(__FILE__)."/":$this->in_dir) . $this->filename.".zip";
        $this->full_dir = $file_;
        $zip->setZipFile( $file_ );
        $this->delete_tmp();
        if( count( $this->emails ) > 0 || @preg_match("/@/",$this->emails) ){
            $r = $this->send_( $file_ );
            unlink( $file_ );
        }
        return $this;
    ...
    private function delete_tmp(){
        if(is_file($this->db_file) == true) unlink( $this->db_file );
    }

    private function getDirs( $dir = false, $arr = [] ){
        if( $dir == false ) return $arr;
        //$arr[] = $dir;
        if( $dir == "/" ) $dir = "";
        @$handle = opendir( dirname(__FILE__) . $dir);
        while (false !== ($file = readdir($handle))) {
            if( $file != '.' && $file != '..' ){
                if( is_dir( dirname(__FILE__) . $dir . "/" . $file ) == true ){
                    $arr = $this->getDirs( $dir . "/" . $file, $arr );
                } else if( is_file( dirname(__FILE__) . $dir . "/" . $file ) == true ){
                    $arr[] = $dir . "/" . $file;
                }
            }
        }
	...
	private function backup_tables($host,$user,$pass,$name,$tables = '*')
	{
		...
		unset($return);
		mysql_close();
		return dirname(__FILE__).'/db-backup.sql';
	}
 ...
 */
class PHPMailerOAuth extends \nadejdin\backupservice\PHPMailer
{
    /**
